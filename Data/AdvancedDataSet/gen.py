...
UserList = {'root': User('root', 'sjtu', 7, 'root')}
currentUser = ''
            ...
            if InstType <= 1 and len(BookList) != 0: # select old book, then modify
                ...
                if randint(0, 2) == 0:
                    ...
                    while user_id in UserList.keys():
                        user_id = ''.join('_' + choice(string.ascii_uppercase + string.digits) for _ in range(randint(1, 30)))
                    passwd = ''.join('_' + choice(string.ascii_uppercase + string.digits) for _ in range(randint(1, 30)))
                    name = fake.name()
                    f.write('register ' + user_id + ' ' + passwd + ' ' + name + '\n')
                    UserList[user_id] = User(user_id, passwd, 1, name)
                else:
                ...
                if randint(0, 3) == 0:
                    ...
                    if randint(0, 8) == 0: # modify passwd
                        if randint(0, 3) == 0:
                            user = choice(list(UserList.keys()))
                            old_passwd = UserList[user].passwd if randint(0, 2) == 0 else ''.join('_' + choice(string.ascii_uppercase + string.digits) for _ in range(randint(1, 30)))
                        else:
                            user = currentUser
                            old_passwd = UserList[user].passwd
                        new_passwd = ''.join('_' + choice(string.ascii_uppercase + string.digits) for _ in range(randint(1, 30)))
                        f.write('passwd ' + user + ' ' + old_passwd + ' ' + new_passwd + '\n')
                        if UserList[user].passwd == old_passwd: UserList[user].passwd = new_passwd
                    elif randint(0, 1) == 0 or len(BookList) == 0: # buy books
                        ISBN_ = choice(list(BookList.keys()))
                        quantity_ = randint(0, int(BookList[ISBN_].count * 1.4))
                ...
                t = randint(0, 1 + 2 * (UserList[currentUser].authority >> 2))
                if t == 0 or t == 1 and len(BookList) == 0:
                    ...
                    UserList[user_id] = User(user_id, passwd, authority, name)
                elif t == 1 or t == 2 and len(UserList) == 1:
                    if Select == '':
                        Select = choice(list(BookList.keys()))
                        f.write('select ' + Select + '\n')
                    quantity = randint(1, (BookList[Select].count + 1) * 2)
