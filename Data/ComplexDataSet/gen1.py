...
for i in range(1, TotalTestCaseNumber + 1):
    ...
    with open(str(i) + '.in', 'w') as f:
        ...
        for j in range(1, InstPerCase + 1):
            ...
            if InstType <= 1 and len(BookList) != 0 and (currentUser != '' and (UserList[currentUser].authority & 3) != 0 or randint(0, 10) == 0): # select old book, then modify
                ...
                if currentUser != '' and (UserList[currentUser].authority & 3) != 0:
                    ...
                    Select = Select_
            elif InstType <= 2 and len(BookList) < MaxBookCount and (currentUser != '' and (UserList[currentUser].authority & 3) != 0 or randint(0, 10) == 0): # create a new book
                Select_ = Select
                Select = fake.isbn13()
                f.write('select ' + Select +'\n')
                name_ = choice(words)
